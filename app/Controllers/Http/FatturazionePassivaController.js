...
class FatturazionePassivaController {
  ...
  async xlsxToJson(file){
    ...
    var workbook = XLSX.readFile(Env.get('FATTURAZIONE_PATH') + '/' + file);
    ...
    for(let i in data){
      var dateStart = moment(data[i]['Inizio_Periodo_Fatturazione'],'DD/MM/YYYY');
      var dateEnd = moment(data[i]['Fine_Periodo_Fatturazione'],'DD/MM/YYYY');
      ...
      let oneri =  (data[i]['Totale_ONERI'].toFixed(5) / timeValues.length).toString().replace('.',',')
      for(let c in timeValues) {
        newArray.push({...data[i],competenza_mese: timeValues[c].slice(-2),competenza_anno: timeValues[c].slice(0, 4),oneri_divided: oneri})
      }
    }
    var xlsxFile = await json2xlsx(newArray,{
      fieldNames: Object.keys(newArray[0])
  });
    await fsExtra.writeFileSync(`${Env.get('FATTURAZIONE_PATH')}/${esitoName}`, xlsxFile, 'binary');
    return esitoName
}
  ...
  async start({response,request}) {
    try {
        ...
        await xlsxFile.move(Env.get('FATTURAZIONE_PATH'), {name:xlsx_name, overwrite: true })
        const esitoName = await this.xlsxToJson(xlsx_name)
        const buffer = fsExtra.readFileSync(Env.get('FATTURAZIONE_PATH')+ '/'+ esitoName);
        fsExtra.unlinkSync(Env.get('FATTURAZIONE_PATH')+ esitoName)
        fsExtra.unlinkSync(Env.get('FATTURAZIONE_PATH')+ xlsx_name)
  ...
  async esportazione({response,request}){
    try {
      var {fornitore,fatturazione_file} = request.all()
      ...
      let query = ""
      if(fornitore) {
        query = `select left("POD",14) as POD,cast("MESE_COMPETENZA" as integer) as MESE,cast("ANNO_COMPETENZA" as integer) as ANNO ,
        coalesce(ROUND(sum(cast(replace("IMP_A_UC_TOTALE" ,',','.')  as numeric)),3),0) as TOT_ONERI,"FORNITORE" from fatturazione_passiva 
        where "POD" in (${list})  and "FORNITORE" = '${fornitore}'
        group by "MESE_COMPETENZA","ANNO_COMPETENZA", left("POD",14),"FORNITORE" 
        order by left("POD",14),cast("ANNO_COMPETENZA" as integer) ,cast("MESE_COMPETENZA" as integer),"FORNITORE"`
      }else {
        query = `select left("POD",14) as POD,cast("MESE_COMPETENZA" as integer) as MESE,cast("ANNO_COMPETENZA" as integer) as ANNO ,
        coalesce(ROUND(sum(cast(replace("IMP_A_UC_TOTALE" ,',','.')  as numeric)),3),0) as TOT_ONERI,"FORNITORE" from fatturazione_passiva 
        where "POD" in (${list})
      ...
      if(result.rows.length >0) {
        result.rows.map(el=>{
          if(el.tot_oneri) el.tot_oneri = Number(el.tot_oneri).toFixed(2).toString().replace('.',',')
        })
  ...
  async getStatusImport({response}){
    try {
      const fatt = await FatturazioneP.query().whereNot('deleted','true').where('status','=','in lavorazione').getCount()
  ...
  async normalizzazione({response,request}){
    ...
    async insert({response,request}){
        try {
          ...
          await this.generateCsv(fornitore,fatturazione_file,fattLog,getConverterName)
    ...
    async generateCsv(fornitore,csv_name,fattLog,getConverterName){
      const trx = await Database.connection('fatturazionepassiva').beginTransaction()
        try{
          ...
          console.log("posti",positionObject)
