...
const name_controller = 'RcuGasCcController'
...
class RcuGasCcController {
  ...
  async statusCache({response,request}){
    try {
      ...
      .raw(`select "MESE","ANNO" from ${tenant}.gas_cc ec order by "ANNO" desc,"MESE" desc limit 1 `)
  ...
  async graficoTotaleDetailPerditaPod({request,response}){
    try {
      ...
      let queryBuilder = `select count(distinct(left("COD_PDR",14))) as totale,"MESE","ANNO" from ${tenant}.gas_cc ec`
      ...
      queryBuilder += ` where left("COD_PDR",14) in 
        (
          select distinct(left("COD_PDR",14)) as COD_PDR from ${tenant}.gas_cc fr2 
          where "ANNO" = ? and "MESE" = ? 
          and left("COD_PDR",14) not in (select left("COD_PDR",14) from ${tenant}.gas_cc fr2 
          where "MESE" = ? and "ANNO" = ?)
  ...
  async _graficoATPPFunc(request,tenant){
    try {
      ...
      let currentDate = `${anno}-${mese}-01`
      let keyRedis = `_graficoATPPFunc_RcuGasCcController_${tenant}_${annopartenza}_${mese}_${anno}`
      ...
      .raw(`select "MESE","ANNO" from ${tenant}.gas_cc ec order by "ANNO" asc,"MESE" asc limit 1 `)
      ...
      do {
        ...
        let queryBuilderOggi = `select count(distinct(left("COD_PDR",14))) as totale from  ${tenant}.gas_cc ec where 
        left("COD_PDR",14) in 
           (
             select distinct(left("COD_PDR",14)) as COD_PDR from  ${tenant}.gas_cc fr2 where "ANNO" = ? and "MESE" = ? 
             and left("COD_PDR",14) not in (select left("COD_PDR",14) from  ${tenant}.gas_cc fr2 where "MESE" = ? and "ANNO" = ?)
           ) and to_date(concat("ANNO","MESE"),'yyyymm') = ?`
        
        let queryBuilderAtt = `select count(distinct(left("COD_PDR",14))) as totale ,"MESE","ANNO" from  ${tenant}.gas_cc fr2
        where "ANNO" = ? and "MESE" = ?
        and left("COD_PDR",14) not in (select left("COD_PDR",14) from ${tenant}.gas_cc fr2 where "MESE" = ? and "ANNO" = ?) 
        ...
        if(tenant == 'ugm' && typeof(invio_fatture) != 'undefined' && invio_fatture != null || typeof(modalita_pagamento) != 'undefined' && modalita_pagamento != null || listaAgenti.length > 0) {
          queryBuilderAtt = `select count(distinct(left("COD_PDR",14))) as totale ,"MESE","ANNO" from  ${tenant}.gas_cc fr2
          ...
          queryBuilderOggi = `select count(distinct(left("COD_PDR",14))) as totale from  ${tenant}.gas_cc ec 
  ...
  async _tableTAFunc(request,tenant) {
    try {
      ...
      let keyRedis = `_tableTAFunc_RcuGasCcController_${tenant}_${annopartenza}_${mese}_${anno}_${limitElements}`
      if(tenant == 'ugm' && typeof(invio_fatture) != 'undefined' && invio_fatture != null) keyRedis+=`_invio_${invio_fatture}`
      if(tenant == 'ugm' && typeof(modalita_pagamento) != 'undefined' && modalita_pagamento != null) keyRedis+=`_pagamento_${modalita_pagamento}`
      var listaAgenti = []
      if(tenant == 'ugm' && typeof(agente_id) != 'undefined' && agente_id != null) {
	  ...
	  let myquery = `select "MESE" as competenza_mese ,"ANNO" as competenza_anno from  ${tenant}.gas_cc ec `;
	  myquery += `where "ANNO" >= ? `
	  
	  if(agente_id == -1){
		myquery += ` and ( "PIVA" is not null OR ( "CF" LIKE '9%' OR "CF" LIKE '8%') ) `;
	  }
	  if(agente_id == -2){
		myquery += ` and "PIVA" is null and ( "CF" not like '8%' and "CF" not like '9%') `;
	  }
	  
	  myquery += `group by "MESE","ANNO" order by "ANNO" ,"MESE"`
	  
	  
      let query = await Database.connection('rcu')
      .raw(myquery,annopartenza)
      if(query.rows.length > 0 ) {
  ...
  async tableTassoAbbandonoVolumi({request,response}){
    try {
      const {annopartenza,mese,anno,limitElements,invio_fatture,modalita_pagamento,agente_id} = request.all()
      let tenant = request.headers().tenant_gas
      let value = await this._tableTAVolumiFunc({annopartenza,mese,anno,limitElements,invio_fatture,modalita_pagamento,agente_id},tenant)
      return response.send({"status": "success","data": value,"message": `Ritorno di tutti le informazioni necessarie per la dashboard`})
    } catch (error) {
      return response.status(500).send({"status": "error","code": 500,"data": null,"message": error.message})
    }
  ...
  async _tableTAVolumiFunc(request,tenant) {
    try {
      var {annopartenza,mese,anno,limitElements,invio_fatture,modalita_pagamento,agente_id} = request
      let keyRedis = `_tableTAVolumiFunc_RcuGasCcController_${tenant}_${annopartenza}_${mese}_${anno}_${limitElements}`
      if(tenant == 'ugm' && typeof(invio_fatture) != 'undefined' && invio_fatture != null) keyRedis+=`_invio_${invio_fatture}`
      if(tenant == 'ugm' && typeof(modalita_pagamento) != 'undefined' && modalita_pagamento != null) keyRedis+=`_pagamento_${modalita_pagamento}`
      var listaAgenti = []
      if(tenant == 'ugm' && typeof(agente_id) != 'undefined' && agente_id != null) {
      ...
      if(query.rows.length>0) {
        ...
        for(let i in query.rows) {
          ...
          let queryArray = [query.rows[i].competenza_anno,query.rows[i].competenza_mese,prevMonth,prevYear,currentDate]
          let queryBuilder = `select ROUND(sum(cast(replace("PREL_ANNUO_PREV",',','.')  as numeric))) as totale,"MESE","ANNO" from ${tenant}.gas_cc ec`
          if(tenant == 'ugm' && typeof(invio_fatture) != 'undefined' && invio_fatture != null || typeof(modalita_pagamento) != 'undefined' && modalita_pagamento != null || listaAgenti.length > 0 ) {
            queryBuilder += ` left join ${tenant}.gas_datamax_filtered t3 on left(t3.cod_pdr,14) = left(ec."COD_PDR",14)`
          }
          if(tenant == 'ugm' && listaAgenti.length > 0) {
  ...
  async _andamentoADVFunc(request,tenant) {
    try {
      var {annopartenza,mese,anno,invio_fatture,modalita_pagamento,agente_id} = request
      let keyRedis = `_andamentoADVFunc_RcuGasCcController_${tenant}_${annopartenza}_${mese}_${anno}`
      if(tenant == 'ugm' && typeof(invio_fatture) != 'undefined' && invio_fatture != null) keyRedis+=`_invio_${invio_fatture}`
      if(tenant == 'ugm' && typeof(modalita_pagamento) != 'undefined' && modalita_pagamento != null) keyRedis+=`_pagamento_${modalita_pagamento}`
      let listaAgenti = []
      if(tenant == 'ugm' && typeof(agente_id) != 'undefined' && agente_id != null) {
      ...
      let queryParcoTotale = `select ROUND(sum(cast(replace("PREL_ANNUO_PREV",',','.')  as numeric))) as totale,"ANNO","MESE" from ${tenant}.gas_cc fr `
      if(tenant == 'ugm' && typeof(invio_fatture) != 'undefined' && invio_fatture != null || typeof(modalita_pagamento) != 'undefined' && modalita_pagamento != null || listaAgenti.length > 0) {
      ...
      do {
        ...
        let queryBuilder = `select ROUND(sum(cast(replace("PREL_ANNUO_PREV",',','.')  as numeric))) as totale ,"MESE","ANNO" from  ${tenant}.gas_cc fr`
        if(tenant == 'ugm' && typeof(invio_fatture) != 'undefined' && invio_fatture != null || typeof(modalita_pagamento) != 'undefined' && modalita_pagamento != null || listaAgenti.length >0 ) {
	  ...
	  if(agente_id == -2){
        ...
        queryBuilder += ` and left("COD_PDR",14) not in (select left("COD_PDR",14) from ${tenant}.gas_cc fr2 where "MESE" = ? and "ANNO" = ?) 
          group by "MESE","ANNO" order by "MESE","ANNO"`
  ...
  async _andamentoADPFunc(request,tenant) {
    try {
      var {annopartenza,mese,anno,invio_fatture,modalita_pagamento,agente_id} = request
      let keyRedis = `_andamentoADPFunc_RcuGasCcController_${tenant}_${annopartenza}_${mese}_${anno}`
      if(tenant == 'ugm' && typeof(invio_fatture) != 'undefined' && invio_fatture != null) keyRedis+=`_invio_${invio_fatture}`
      if(tenant == 'ugm' && typeof(modalita_pagamento) != 'undefined' && modalita_pagamento != null) keyRedis+=`_pagamento_${modalita_pagamento}`
      let listaAgenti = []
      if(tenant == 'ugm' && typeof(agente_id) != 'undefined' && agente_id != null) {
      ...
      let objectPercent = {series:[{name:'Attivazioni',type: 'line',data: []},
      {name:'Disattivazioni',type: 'line',data:[]}],labels:[]}
      let queryParcoTotale = `select count(distinct(left("COD_PDR",14))) as totale,"ANNO","MESE" from ${tenant}.gas_cc fr `
      if(tenant == 'ugm' && typeof(invio_fatture) != 'undefined' && invio_fatture != null || typeof(modalita_pagamento) != 'undefined' && modalita_pagamento != null || listaAgenti.length > 0 ) {
        queryParcoTotale += `left join ${tenant}.gas_datamax_filtered t3 on left(t3.cod_pdr,14) = left(fr."COD_PDR",14) `
      }
      if(tenant == 'ugm' && listaAgenti.length > 0) {
      ...
      do {
        ...
        let queryBuilder = `select count(distinct(left("COD_PDR",14))) as totale,"ANNO","MESE" from ${tenant}.gas_cc fr `
        if(tenant == 'ugm' && typeof(invio_fatture) != 'undefined' && invio_fatture != null || typeof(modalita_pagamento) != 'undefined' && modalita_pagamento != null || listaAgenti.length > 0 ) {
          queryBuilder += ` left join ${tenant}.gas_datamax_filtered t3 on left(t3.cod_pdr,14) = left(fr."COD_PDR",14) `
        }
        if(tenant == 'ugm' && listaAgenti.length > 0) {
  ...
  async deleteRowField({response,request}){
    ...
    try {
      ...
      var { id } = request.all();
      await trx
        .table(request.headers().tenant_gas + ".gas_cc")
        .where("gas_cc_histories_id", id)
        .delete();
      await trx.table(`${request.headers().tenant_gas}.gas_cc_histories`).where("id", id).update({ deleted: true, delete_owner: tokenUser.username, delete_owner_ip: tokenUser.ip_address });
      await trx.commit();
      return response.send({ status: "success", data: await Database.connection("rcu").table(`${request.headers().tenant_gas}.gas_cc_histories`).where("deleted", false).orderBy("created_at", "desc"), message: null });
    } catch (error) {
      trx.rollback;
      return response.status(500).send({ status: "error", code: 500, data: null, message: error.message });
    }
        ...
        if(id) {
          ...
          .raw(
            `SELECT left("COD_PDR" ,14)as "COD_PDR", "COD_REMI", "DISALIMENTABILITA", "DATA_INIZIO_FOR", "DATA_FINE_FOR", "RAGIONE_SOCIALE_UDD", "PIVA_UDD", "RAGIONE_SOCIALE_DD", "PIVA_DD", "CF", "PIVA", "NOME", "COGNOME", "RAGIONE_SOCIALE_DENOMINAZIONE", "RESIDENZA", "ALIQUOTA_IVA", "ALIQUOTA_ACCISE", "ADDIZ_REGIONALE", "SETT_MERCEOLOGICO", "TRATTAMENTO", "PREL_ANNUO_PREV", "COD_PROF_PREL_STD", "MATR_MIS", "CLASSE_GRUPPO_MIS", "TELEGESTIONE", "PRE_CONV", "MATR_CONV", "N_CIFRE_CONV", "COEFF_CORR", "BONUS", "BS_DATA_INIZIO", "BS_DATA_FINE", "BS_DATA_RINNOVO", "MESE", "ANNO"
              from ${request.headers().tenant_gas}.gas_cc where gas_cc_histories_id = ? 
            `,[id])
        }else {
          jsonData =  await Database.connection('rcu')
          .raw(
            `SELECT left("COD_PDR" ,14)as "COD_PDR", "COD_REMI", "DISALIMENTABILITA", "DATA_INIZIO_FOR", "DATA_FINE_FOR", "RAGIONE_SOCIALE_UDD", "PIVA_UDD", "RAGIONE_SOCIALE_DD", "PIVA_DD", "CF", "PIVA", "NOME", "COGNOME", "RAGIONE_SOCIALE_DENOMINAZIONE", "RESIDENZA", "ALIQUOTA_IVA", "ALIQUOTA_ACCISE", "ADDIZ_REGIONALE", "SETT_MERCEOLOGICO", "TRATTAMENTO", "PREL_ANNUO_PREV", "COD_PROF_PREL_STD", "MATR_MIS", "CLASSE_GRUPPO_MIS", "TELEGESTIONE", "PRE_CONV", "MATR_CONV", "N_CIFRE_CONV", "COEFF_CORR", "BONUS", "BS_DATA_INIZIO", "BS_DATA_FINE", "BS_DATA_RINNOVO", "MESE", "ANNO"
              from ${request.headers().tenant_gas}.gas_cc where  "MESE" = ? and "ANNO" = ?
            `,[mese,anno])
        }
  ...
  async checkSintesi({ response, request }) {
    try {
      ...
      let find = Database.connection("rcu").table(`${request.headers().tenant_gas}.gas_cc_histories`).where("deleted", false);
  ...
  async tableIncPod({request,response}){
    try {
      ...
      .raw(
      `select "RAGIONE_SOCIALE_DD" , count("RAGIONE_SOCIALE_DD") as totale ,"MESE" ,"ANNO" from ${request.headers().tenant_gas}.gas_cc fr  where "ANNO" = ? or 
      ("ANNO" = ? and "MESE" = '12' and "RAGIONE_SOCIALE_DD" in (select distinct("RAGIONE_SOCIALE_DD") from ${request.headers().tenant_gas}.gas_cc fr where "ANNO" = ?))
  ...
  async tableIncVolumi({request,response}){
    try {
      ...
      or ("ANNO" = ? and "MESE" = '12' and "RAGIONE_SOCIALE_DD" in (select distinct("RAGIONE_SOCIALE_DD") from ${request.headers().tenant_gas}.gas_cc fr where "ANNO" = ?)) group by "RAGIONE_SOCIALE_DD","MESE","ANNO" order by "MESE", "RAGIONE_SOCIALE_DD"`,[anno,anno-1,anno])
      getTotalForFornitore = getTotalForFornitore.rows
      const queryBuilder = await this._setTableIncAnnuale(getTotalForFornitore,anno)
      return response.send({"status": "success","data": queryBuilder,"message": `Ritorno di tutti le informazioni necessarie per la dashboard`})
    } catch (error) {
  ...
  async tableVolumiAnnuale({request,response}){
    try {
      const {anno} = request.all()
      let getTotalForFornitore = await Database.connection('rcu')
      .raw(`select "RAGIONE_SOCIALE_DD","MESE",coalesce(ROUND(sum(cast(replace("PREL_ANNUO_PREV",',','.')  as numeric)) ,2),0) as TOTALE from ${request.headers().tenant_gas}.gas_cc where "ANNO" = ? group by "RAGIONE_SOCIALE_DD","MESE" order by "MESE", "RAGIONE_SOCIALE_DD"`,[anno])
      getTotalForFornitore = getTotalForFornitore.rows
  ...
  async tablePodAnnuale({request,response}){
    try {
      ...
      let getTotalForFornitore = await Database.connection('rcu')
      .raw(
      `select "RAGIONE_SOCIALE_DD" , count("RAGIONE_SOCIALE_DD") as totale ,"MESE" ,"ANNO" from ${request.headers().tenant_gas}.gas_cc fr  where "ANNO" = ? 
  ...
  async sintesiInformation({request,response}) {
    try {
      ...
      let prevMese = mese-1
      let prevAnno = anno
      let MinMonth = await await Database.connection('rcu').raw(`select min("MESE") from ${request.headers().tenant_gas}.gas_cc ec where "ANNO" = ?`,[anno])
  ...
  async tableInformation({request,response}){
    try {
      ...
      from ${request.headers().tenant_gas}.gas_cc fr where "ANNO" = ? and "MESE" = ? group by "RAGIONE_SOCIALE_DD" order by pod desc`,[anno,mese])
      return response.send({"status": "success","data": query.rows,"message": `Ritorno di tutti le informazioni necessarie per la dashboard`})
    } catch (error) {
      return response.status(500).send({"status": "error","code": 500,"data": null,"message": error.message})
    }
  ...
  async esportazione({request,response}){
    try {
      ...
      .raw(`select "RAGIONE_SOCIALE_DD","MESE",sum(cast(replace("PREL_ANNUO_PREV",',','.')  as float))  as TOTALE from ${request.headers().tenant_gas}.gas_cc where "ANNO" = ? group by "RAGIONE_SOCIALE_DD","MESE" order by "MESE"`,[anno])
  ...
  async zipFile(zip_file_name,mese,anno,tokenUser,request){
    try {
      ...
      let buffer = await fsExtra.readFileSync(Env.get('RCU_PATH') + zip_file_name)
      ...
      let query = await Database.connection('rcu')
      .raw(`select "MESE","ANNO" from ugm.gas_cc ec order by "ANNO" desc,"MESE" desc limit 1 `)
      if(query.rows.length > 0) {
        let RedisController = use(`App/Controllers/Http/RedisRouteController`)
        const RedisClass = new RedisController()
  ...
  async generateCsv(csv,mese,anno,tokenUser,request){
    ...
    const trx = await Database.connection('rcu').beginTransaction()
    let fattLog = null
    let checkExist = await Database.connection("rcu").table(`${request.headers().tenant_gas}.gas_cc_histories`).where("note", csv.path)
    .where(inner => {inner.where('status','completato').orWhere('status','in lavorazione')}).whereNot('deleted',true);
    console.log(checkExist)
    try{
      if(checkExist.length == 0){
      fattLog = await Database.connection("rcu").table(`${request.headers().tenant_gas}.gas_cc_histories`).insert({ note: csv.path, mese: mese, anno: anno, importati: 0, status: "in lavorazione", owner: tokenUser.username, owner_ip: tokenUser.ip_address }).returning("id");
         var counterTotal = 0
          ...
          for(let i in datafile) {
            ...
            if(Object.keys(tempObject).length>0){
              ...
              if (i == 1) {
                let check = trx.table(request.headers().tenant_gas + ".gas_cc");
              ...
              // if(tempObject.PIVA_UDD == '03163990611' || tempObject.PIVA_UDD == '3163990611') {
                ...
                await trx.table(request.headers().tenant_gas + ".gas_cc").insert({ ...tempObject, gas_cc_histories_id: fattLog[0] });
              // } 
            }
          }
          await trx.table(`${request.headers().tenant_gas}.gas_cc_histories`).where("id", fattLog[0]).update({ importati: counterTotal, status: "completato" });
          trx.commit();
      ...
      console.log("e",e)
      trx.rollback();
      await Database.connection("rcu").table(`${request.headers().tenant_gas}.gas_cc_histories`).table(`${request.headers().tenant_ee}.gas_cc_histories`).where("id", fattLog[0]).update({ importati: 0, status: "in errore" });
      throw e
    }
  }
  ...
  async getHistory({response,request}){
    try {
      var fatt = await Database.connection("rcu").table(`${request.headers().tenant_gas}.gas_cc_histories`).where("deleted", false).orderBy("id", "desc")
  ...
  async getStatusImport({response,request}){
    try {
      const fatt = await Database.connection("rcu").table(`${request.headers().tenant_gas}.gas_cc_histories`).where("deleted", false).where("status", "=", "in lavorazione").getCount();
